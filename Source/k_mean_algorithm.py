...
def startKmeans(someK,im):
    ...
    while not converged(centroids, old_centroids) and i <= 20:
		...
		clusters = assignPixels(centroids,im) 						#Assign each pixel in the image to their respective centroids
...
#
def drawWindow(dir_mainfolder,result,im,num):
    img_width, img_height = im.size
    px = im.load()
    ...
    for x in range(img.size[0]):
		for y in range(img.size[1]):
			...
			p[x, y] = RGB_value
    img.save(dir_mainfolder+"/img_kmean_handwrite/"+str(num)+".jpg")

def k_mean_main(dir_mainfolder,set_k):
    k_input = set_k
    num = 1
    cv_img = []
    ################
    ...
    for img2 in glob.glob(dir_mainfolder + "/*.jpg"):
        ...
        cv_img.append(im)
    for im in cv_img:
        result = startKmeans(k_input,im)
        drawWindow(dir_mainfolder,result,im,num)
        num=num+1
   
def k_mean_simple(file_path,set_k):
    k_input = set_k
